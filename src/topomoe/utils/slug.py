"""
A script to generate a random adjective-noun slug.

Copied from: https://github.com/nas5w/random-word-slugs
"""
...
# wordlist for generating random adjective-noun slugs
# copied from: https://github.com/nas5w/random-word-slugs/blob/master/words.ts
WORDS = {
    "noun": [
...
def random_slug(seed: int = None, words: Dict = WORDS) -> str:
    if seed:
        random.seed(seed)
    adj = random.choice(words["adjective"])["word"]
    noun = random.choice(words["noun"])["word"]
    return f"{adj}-{noun}"
...
if __name__ == "__main__":
    ...
    print(random_slug(seed=args.seed))
